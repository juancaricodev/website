...
- [ ] 2.2 `Projects.module.css` — Cards: same surface/border/radius 12px/hover shadow as 2.1. Buttons (`.links a`): hover `background: var(--color-brand-600)` → `var(--color-brand-500)`; `:focus-visible`: `outline: 2px solid var(--color-brand-500)`, offset 2px. Badges keep light variant (brand-100/brand-800). ALSO restore lost mobile responsiveness (regression from migration): `@media only screen and (max-width: 425px) { .links { flex-direction: column; align-items: flex-start; } }` — mirrors vanilla `.work__item-links` behavior.
